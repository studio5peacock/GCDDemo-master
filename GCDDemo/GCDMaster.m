...
+ (void)initialize {
    ...
    serialQueue = dispatch_queue_create("com.5peacock.SerialQueue", NULL);
...
static void (^counterBlock)(int,char, NSUInteger , GCDMaster*) = ^(int count, char letter, NSUInteger blockNumber, GCDMaster *delegate){
    ...
    //@synthronized();,it  will lock the pthread and unlock the pthread to make sure it will be implemented.
    dispatch_async(dispatch_get_main_queue(), ^{
        [delegate.delegate GCDMaster:delegate blockNumber:blockNumber changedStatus:BlockStatusFinished];
        if (++delegate.blockCounter == 4)
            [delegate updateUI];
    });
};
